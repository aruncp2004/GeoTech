...
import { ArrowLeft, Pencil, X, FileText, FolderOpen, MapPin, ExternalLink } from 'lucide-react'
import { toast } from 'sonner'
import type { Sample, SampleType } from '@/types'

// ── Courier tracking ──────────────────────────────────────────────────────────
const KNOWN_COURIERS: { keywords: string[]; url: (awb: string) => string }[] = [
  { keywords: ['blue dart', 'bluedart'], url: (awb) => `https://www.bluedart.com/tracking?trackFor=0&track=shipment&trackNo=${awb}` },
  { keywords: ['dtdc'], url: (awb) => `https://www.dtdc.in/tracking.asp?Ttype=awb&strCnno=${awb}` },
  { keywords: ['delhivery'], url: (awb) => `https://www.delhivery.com/track/package/${awb}` },
  { keywords: ['xpressbees'], url: (awb) => `https://www.xpressbees.com/shipment/tracking?awbNo=${awb}` },
  { keywords: ['ecom express', 'ecomexpress'], url: (awb) => `https://ecomexpress.in/tracking/?awb_field=${awb}` },
  { keywords: ['india post', 'indiapost', 'speed post'], url: (awb) => `https://www.indiapost.gov.in/_layouts/15/dop.portal.tracking/trackconsignment.aspx?AWBNo=${awb}` },
  { keywords: ['fedex'], url: (awb) => `https://www.fedex.com/en-in/tracking.html?tracknumbers=${awb}` },
  { keywords: ['dhl'], url: (awb) => `https://www.dhl.com/in-en/home/tracking.html?tracking-id=${awb}` },
  { keywords: ['professional courier', 'tpc'], url: (awb) => `https://tpcindia.com/tracking/?id=${awb}` },
  { keywords: ['ekart', 'flipkart'], url: (awb) => `https://ekartlogistics.com/shipmenttrack/${awb}` },
  { keywords: ['amazon'], url: (awb) => `https://track.amazon.in/tracking/${awb}` },
]

function getTrackingInfo(courierName: string, awb: string): { url: string; isKnown: boolean } | null {
...
function UpdateCourierForm({
  ...
  const handleSave = async () => {
    if (!courier) { toast.error('Please enter a courier name'); return }
    if (!dispatchDate) { toast.error('Please enter the dispatch date'); return }
    setSaving(true)
    try {
      await updateSample.mutateAsync({
        id: sampleId,
        updates: { courier_name: courier, awb_number: awb, pickup_date: dispatchDate, status: 'picked_up' },
      })
    ...
    <div className="space-y-3">
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <Input placeholder="e.g. 76202430" value={awb} onChange={(e) => setAwb(e.target.value)} className="mt-1" />
        </div>
        <div className="sm:col-span-2">
          ...
          <p className="text-xs text-muted-foreground mt-1">Enter the actual date you gave the parcel to the courier office</p>
        </div>
      </div>
...
export default function ParcelDetailPage() {
  ...
  const trackingInfo = getTrackingInfo(sample.courier_name || '', sample.awb_number || '')
          ...
          {!editing && (
            <div className="text-sm divide-y">
              {[
                ...
                { label: 'AWB / Tracking no.', value: sample.awb_number || 'Not assigned' },
                {
                  label: 'Date of sending',
                  value: sample.pickup_date
                    ? new Date(sample.pickup_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })
                    : '—'
                },
                {
                  label: 'Submitted on',
                  value: new Date(sample.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }),
                },
                ...(sample.received_at ? [{
                  label: 'Received on',
                  value: new Date(sample.received_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }),
                }] : []),
                ...
                <div key={item.label} className="flex justify-between py-2.5 gap-4">
                  <span className="text-muted-foreground flex-shrink-0 min-w-[130px]">{item.label}</span>
                  ...
                  {trackingInfo.isKnown ? (
                    ...
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">
                        Direct tracking not available for <span className="font-medium text-foreground">{sample.courier_name}</span>
                      </p>
                      <a
                        href={trackingInfo.url}
                        target="_blank"
            ...
            <div className="space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={editForm.sample_type} onChange={(e) => setEditForm(f => ({ ...f, sample_type: e.target.value }))}

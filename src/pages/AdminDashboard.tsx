...
export default function AdminDashboard() {
  ...
  const atCourierCount = samples.filter(s => s.status === 'at_courier').length
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-6 py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-0.5">All parcels across all supervisors</p>
          </div>
          ...
          <div className="bg-yellow-50 border border-yellow-300 rounded-xl p-4 mb-6 flex items-center gap-3 animate-pulse">
            ...
            <div>
              <p className="font-bold text-yellow-800 text-sm">
                {atCourierCount} parcel{atCourierCount > 1 ? 's' : ''} at courier hub
              </p>
              <p className="text-yellow-700 text-xs mt-0.5">
                {atCourierCount > 1 ? 'These parcels have' : 'This parcel has'} reached the courier office — action needed
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: "Booked", value: samples.filter(s => s.status === "booked").length, color: "bg-blue-50 text-blue-700" },
            {
              ...
              pulse: samples.filter(s => s.status === "at_courier").length > 0,
            },
            { label: "Received at Lab", value: samples.filter(s => s.status === "received").length, color: "bg-green-50 text-green-700" },
          ].map((card) => (
            <div key={card.label} className={`${card.color} rounded-xl p-4 border relative overflow-hidden`}>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr key={s.id} className={`border-b transition-colors ${
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          ...
                          <Link to={`/admin/parcel/${s.id}`} className="font-black text-primary font-mono hover:underline">
                            {s.sample_id}
                          </Link>
                        </div>
                      </td>
                      ...
                      <td className="p-4 hidden xl:table-cell">
                        {s.received_at ? (
                          <span className="text-green-600 font-medium">{formatDate(s.received_at)}</span>
                        ) : (
                  ...
                  {filtered.length === 0 && (
                    <tr>
                      <td colSpan={9} className="p-12 text-center text-muted-foreground">No parcels found.</td>
                    </tr>
                  )}
                </tbody>
              </table>
